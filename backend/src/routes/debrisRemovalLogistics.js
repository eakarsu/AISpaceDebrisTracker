...
// Active debris removal logistics with payload capacity/cost optimization
// Feature: debris-removal-logistics
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Active debris removal logistics with payload capacity/cost optimization.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'debris-removal-logistics', payload, result);
    res.json({ success: true, feature: 'debris-removal-logistics', ...result });
  } catch (err) {
    console.error('debris-removal-logistics analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Active debris removal logistics with payload capacity/cost optimization. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'debris-removal-logistics', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'debris-removal-logistics', title: 'Active debris removal logistics with payload capacity/cost optimization', project: 'AISpaceDebrisTracker' });
});
