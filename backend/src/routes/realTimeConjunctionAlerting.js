...
// Real-time conjunction alerting via webhooks and pager-style escalation
// Feature: real-time-conjunction-alerting
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AISpaceDebrisTracker" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Real-time conjunction alerting via webhooks and pager-style escalation.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'real-time-conjunction-alerting', payload, result);
    res.json({ success: true, feature: 'real-time-conjunction-alerting', ...result });
  } catch (err) {
    console.error('real-time-conjunction-alerting analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AISpaceDebrisTracker". Return strict JSON only.',
      `Feature: Real-time conjunction alerting via webhooks and pager-style escalation. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'real-time-conjunction-alerting', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'real-time-conjunction-alerting']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'real-time-conjunction-alerting', title: 'Real-time conjunction alerting via webhooks and pager-style escalation', project: 'AISpaceDebrisTracker' });
});
