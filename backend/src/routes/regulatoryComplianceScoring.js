...
// Regulatory compliance scoring against national/international guidelines
// Feature: regulatory-compliance-scoring
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Regulatory compliance scoring against national/international guidelines.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'regulatory-compliance-scoring', payload, result);
    res.json({ success: true, feature: 'regulatory-compliance-scoring', ...result });
  } catch (err) {
    console.error('regulatory-compliance-scoring analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Regulatory compliance scoring against national/international guidelines. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'regulatory-compliance-scoring', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'regulatory-compliance-scoring']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'regulatory-compliance-scoring', title: 'Regulatory compliance scoring against national/international guidelines', project: 'AISpaceDebrisTracker' });
});
