...
// Multi-mission optimizer recommending constellation reconfigurations
// Feature: multi-mission-optimizer
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Multi-mission optimizer recommending constellation reconfigurations.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'multi-mission-optimizer', payload, result);
    res.json({ success: true, feature: 'multi-mission-optimizer', ...result });
  } catch (err) {
    console.error('multi-mission-optimizer analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Multi-mission optimizer recommending constellation reconfigurations. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'multi-mission-optimizer', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'multi-mission-optimizer', title: 'Multi-mission optimizer recommending constellation reconfigurations', project: 'AISpaceDebrisTracker' });
});
