# Space Debris Tracker operations

## Supported operating boundary

The governed path covers orbital asset observation, constrained decision review, approval, provider queueing, receipt feedback, and manual recovery. Provider names are typed adapter contracts, not a claim that a live third-party account is connected. Keep outbound execution disabled until each configured orbital catalog, tracking sensor, mission control, ERP, GIS, weather, maintenance, and notification adapter has passed sandbox contract tests and receipt reconciliation.
...
```sh
node --test backend/src/governance/tests/*.test.js
bash -n start.sh
./start.sh check
```
