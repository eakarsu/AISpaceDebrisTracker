...
export default function CfMultiMissionOptimizerRecommendingConstellationReconfigurations() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-mission-optimizer-recommending-constellation-reconfigurations/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-multi-mission-optimizer-recommending-constellation-reconfigurations', project: 'AISpaceDebrisTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-mission-optimizer-recommending-constellation-reconfigurations/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Multi-mission optimizer recommending constellation reconfigurations</h1>
