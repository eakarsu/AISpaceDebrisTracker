...
// Feature: Sensor fusion combining optical/radar observations via ML to improve conjunction confidence
...
export default function CfSensorFusionCombiningOpticalRadarObservationsVia() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-sensor-fusion-combining-optical-radar-observations-via-ml/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-sensor-fusion-combining-optical-radar-observations-via-ml', project: 'AISpaceDebrisTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-sensor-fusion-combining-optical-radar-observations-via-ml/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Sensor fusion combining optical/radar observations via ML to improve conjunction confidence</h1>
