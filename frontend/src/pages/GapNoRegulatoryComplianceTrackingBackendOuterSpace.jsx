...
export default function GapNoRegulatoryComplianceTrackingBackendOuterSpace() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-regulatory-compliance-tracking-backend-outer-space-treaty/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-regulatory-compliance-tracking-backend-outer-space-treaty', project: 'AISpaceDebrisTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-regulatory-compliance-tracking-backend-outer-space-treaty/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No regulatory compliance tracking backend (Outer Space Treaty, debris mitigation guidelines)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AISpaceDebrisTracker</p>
      </div>
