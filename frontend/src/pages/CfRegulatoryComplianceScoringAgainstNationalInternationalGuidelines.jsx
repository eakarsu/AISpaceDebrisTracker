...
export default function CfRegulatoryComplianceScoringAgainstNationalInternationalGuidelines() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-regulatory-compliance-scoring-against-national-international-guidelines/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-regulatory-compliance-scoring-against-national-international-guidelines', project: 'AISpaceDebrisTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-regulatory-compliance-scoring-against-national-international-guidelines/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Regulatory compliance scoring against national/international guidelines</h1>
