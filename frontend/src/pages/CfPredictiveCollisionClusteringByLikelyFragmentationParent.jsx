...
// Feature: Predictive collision clustering by likely fragmentation parent with mitigation priority
...
export default function CfPredictiveCollisionClusteringByLikelyFragmentationParent() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-predictive-collision-clustering-by-likely-fragmentation-parent-with/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-predictive-collision-clustering-by-likely-fragmentation-parent-with', project: 'AISpaceDebrisTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-predictive-collision-clustering-by-likely-fragmentation-parent-with/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Predictive collision clustering by likely fragmentation parent with mitigation priority</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AISpaceDebrisTracker</p>
      </div>
