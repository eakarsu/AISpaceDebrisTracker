...
export default function CfActiveDebrisRemovalLogisticsWithPayloadCapacity() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-active-debris-removal-logistics-with-payload-capacity-cost/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-active-debris-removal-logistics-with-payload-capacity-cost', project: 'AISpaceDebrisTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-active-debris-removal-logistics-with-payload-capacity-cost/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Active debris removal logistics with payload capacity/cost optimization</h1>
