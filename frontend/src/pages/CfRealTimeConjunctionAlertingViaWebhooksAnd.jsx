...
export default function CfRealTimeConjunctionAlertingViaWebhooksAnd() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-real-time-conjunction-alerting-via-webhooks-and-pager-style-escalation/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-real-time-conjunction-alerting-via-webhooks-and-pager-style-escalation', project: 'AISpaceDebrisTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-real-time-conjunction-alerting-via-webhooks-and-pager-style-escalation/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Real-time conjunction alerting via webhooks and pager-style escalation</h1>
