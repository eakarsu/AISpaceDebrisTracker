...
export default function GapNo3dVisualizationOfDebrisCloudsAt() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-3d-visualization-of-debris-clouds-at-the/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-3d-visualization-of-debris-clouds-at-the', project: 'AISpaceDebrisTracker' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-3d-visualization-of-debris-clouds-at-the/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No 3D visualization of debris clouds at the backend (only frontend stubs)</h1>
